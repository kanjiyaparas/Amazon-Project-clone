...
export default function ImageDialog(props) {
    const {fetchMediaFromBackend, media,setfutereimg} = props
        ...
        <Dialog fullWidth={fullWidth} maxWidth={maxWidth} open={open} onClose={handleClose} sx={{zIndex:"100000000000"}}>
          ...
          <label
            ...
            {media.map((image, index) => (
              ...
              <img key={index} src={image.url} alt={`Image ${index}`} 
                ...
                height:"15.3rem" , 
                objectFit: "cover", 
                border: tmpselectedimg._id === image._id ? '3px dashed #1976d2' : 'none' }}  

                 onClick={() => {
                  if(tmpselectedimg._id !== image._id){
                    settmpselectedimg(image)
                  }else{
                    settmpselectedimg({})
                  }
                 }} />
          ...
          <DialogActions>
            <Button onClick={(image)=>{
              handleClose()
              if(!tmpselectedimg._id){
                setfutereimg({})
              }
            }}>Close</Button>
            <Button onClick={(image)=>{
              if(!tmpselectedimg._id){
                return alert("please select your feature image")
              }
              setfutereimg(tmpselectedimg)
              setOpen(false)
            }}>save</Button>
